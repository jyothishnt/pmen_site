...
var getMarkers = function(map, data, _type) {
	...
	for(var i=0; i<data.length;i++) {
		...
		html += "<h3 style='color: #007fc5;letter-spacing: 1px;'>" + data[i].country + " ("+ data[i].continent +")</h3><table>";
		var head = '';
		for (var key in data[i]) {
			var res = key.match(/latitude|longitude|url|pmen_designation|country|continent/);
			
			if(data[i][key] != "" && key != "latitude" && !res) {
				head = key.capitalize();
				head = head.replace(/_/g,' ');
				if(key == "reference") {
					var url = (data[i]['url'] != "") ? data[i]['url'] : "http://www.mlst.net";
					html += "<tr><td width=\"120px\"><span style='color: #666;'>"+ head +": </span></td><td><span style='color:#333;'>" + 
							"<a href='" + url + "' target='_blank'>" + data[i][key] + "</a></span></td></tr>";
				}
				else {
